...
def _initial_solution(orders, context, origin, interval, horizon,
    ...
    ordered = sorted(orders, key=lambda o: (
        o.fecha_compromiso, o.prioridad, o.fecha_liberacion, -o.cantidad
    ))
...
def optimizar_lns(
    ...
    while iteration < max_iteraciones and time.perf_counter() - started < tiempo_limite_segundos:
        ...
        count = max(1, min(len(orders) - 1, round(len(orders) * porcentaje_destruccion)))
